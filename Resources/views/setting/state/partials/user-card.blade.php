@push('plugin-style')
    <!-- Ekko Lightbox -->
    <link rel="stylesheet" href="{{ asset('modules/admin/plugins/ekko-lightbox/ekko-lightbox.css') }}">
@endpush
        ...
        @if($user->roles->count() > 0)
            ...
            <p class="text-muted">
                @foreach($user->roles as $role)
                    <a href="{{ route('core.settings.roles.show', $role->id) }}">{{ $role->name }}</a>,
                @endforeach
            </p>
            <hr>
        @endif
...
@push('plugin-script')
    <!-- Ekko Lightbox -->
    <script src="{{ asset('modules/admin/plugins/ekko-lightbox/ekko-lightbox.min.js') }}"></script>
@endpush
