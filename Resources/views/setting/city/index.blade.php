...
@section('actions')
    {!! \Html::linkButton('Add Role', 'core.settings.roles.create', [], 'mdi mdi-plus', 'success') !!}
    {!! \Html::bulkDropdown('core.settings.roles', 0, ['color' => 'warning']) !!}
@endsection
        ...
        <div class="card card-default">
            @if(!empty($roles))
                <div class="card-body p-0">
                    {!! \Html::cardSearch('search', 'core.settings.roles.index',
...
'class' => 'form-control', 'id' => 'search', 'data-target-table' => 'role-table']) !!}
                        ...
                        <table class="table table-hover mb-0" id="role-table">
                            ...
                            @forelse($roles as $index => $role)
                                <tr @if($role->deleted_at != null) class="table-danger" @endif>
                                    ...
                                    <td class="text-left">
                                        @if(auth()->user()->can('core.settings.roles.show') || in_array($role->id, auth()->user()->role_ids))
                                            <a href="{{ route('core.settings.roles.show', $role->id) }}">
                                                {{ $role->name }}
                                            </a>
                                        @else
                                            {{ $role->name }}
                                    ...
                                    <td class="exclude-search pr-3 text-center align-middle">
                                        {!! \Html::actionDropdown('core.settings.roles', $role->id, array_merge(['show', 'edit'], ($role->deleted_at == null) ? ['delete'] : ['restore'])) !!}
                                    </td>
                                </tr>
                            @empty
                                <tr>
    ...
    <!-- /.container-fluid -->
    {!! \Modules\Core\Supports\CHTML::confirmModal('Role', ['export','delete', 'restore']) !!}
@endsection
